...
# ============================================
# NEW: FILTER FUNCTIONS FOR CONTRACTS AND POLICIES (ADD THIS)
# ============================================
...
# ============================================
# CUSTOM CSS - FIXED FOR DROPDOWN VISIBILITY
# ============================================
if st.session_state.theme == "light":
    THEME_CSS = f"""
    <style>
...
# ============================================
# DASHBOARD - WITH WORKING FILTERS FOR ALL TABS
# ============================================
    ...
    if work_plans:
        ...
        filtered_work_df['days_left'] = (pd.to_datetime(filtered_work_df['due_date']) - pd.Timestamp.now()).dt.days
        filtered_work_df['status_group'] = filtered_work_df['calculated_progress'].apply(
            lambda x: 'Completed' if x >= 100 else ('In Progress' if x > 0 else 'Not Started')
        )
    else:
        filtered_work_df = pd.DataFrame()
    
    # Apply filters to Contracts - FIXED
    if contracts:
    ...
    # Apply filters to Policies - FIXED
    if policies:
    ...
    with tab_work:
        if not filtered_work_df.empty:
            ...
            with col_chart4:
                ...
                quarterly_data = filtered_work_df.groupby('quarter')['calculated_progress'].mean().reset_index()
                quarterly_data.columns = ['Quarter', 'Avg Progress %']
                ...
                fig = px.bar(quarterly_data, x='Quarter', y='Avg Progress %', color='Avg Progress %',
                            color_continuous_scale='Greens', text='Avg Progress %')
                fig.update_traces(texttemplate='%{text:.1f}%', textposition='outside')
                fig.update_layout(height=350, yaxis_title="Avg Progress %", margin=dict(l=20, r=20, t=30, b=20))
                st.plotly_chart(fig, use_container_width=True)
    ...
    with tab_contracts:
        if not filtered_contracts_df.empty:
            ...
            col1, col2, col3, col4, col5 = st.columns(5)
            with col1:
                st.markdown(f"<div class='kpi-card'><div class='kpi-label'>💰 TOTAL VALUE</div><div class='kpi-value'>KES {total_value/1e6:.1f}M</div><div class='kpi-sub'>Total Contract Value</div></div>", unsafe_allow_html=True)
            with col2:
                st.markdown(f"<div class='kpi-card'><div class='kpi-label'>💸 TOTAL SPENT</div><div class='kpi-value'>KES {total_spent/1e6:.1f}M</div><div class='progress-bar'><div class='progress-fill' style='width:{utilization}%;'></div></div><div class='kpi-sub'>{utilization:.0f}% utilized</div></div>", unsafe_allow_html=True)
            with col3:
                st.markdown(f"<div class='kpi-card'><div class='kpi-label'>✅ ACTIVE</div><div class='kpi-value'>{active}</div><div class='kpi-sub'>Active Contracts</div></div>", unsafe_allow_html=True)
            with col4:
                st.markdown(f"<div class='kpi-card'><div class='kpi-label'>⚠️ EXPIRING SOON</div><div class='kpi-value'>{expiring}</div><div class='kpi-sub'>Within 30 days</div></div>", unsafe_allow_html=True)
            with col5:
                st.markdown(f"<div class='kpi-card'><div class='kpi-label'>⭐ AVG RATING</div><div class='kpi-value'>{avg_performance:.1f}/5</div><div class='kpi-sub'>Vendor Performance</div></div>", unsafe_allow_html=True)
    ...
    with tab_policies:
        if not filtered_policies_df.empty:
            ...
            col1, col2, col3, col4 = st.columns(4)
            with col1:
                st.markdown(f"<div class='kpi-card'><div class='kpi-label'>📜 TOTAL POLICIES</div><div class='kpi-value'>{total_policies}</div><div class='kpi-sub'>All Policies</div></div>", unsafe_allow_html=True)
            with col2:
                st.markdown(f"<div class='kpi-card'><div class='kpi-label'>✅ ACTIVE</div><div class='kpi-value'>{active_policies}</div><div class='kpi-sub'>Currently Active</div></div>", unsafe_allow_html=True)
            with col3:
                st.markdown(f"<div class='kpi-card'><div class='kpi-label'>⚠️ EXPIRING SOON</div><div class='kpi-value'>{expiring_soon}</div><div class='kpi-sub'>Within 90 days</div></div>", unsafe_allow_html=True)
            with col4:
                st.markdown(f"<div class='kpi-card'><div class='kpi-label'>🔴 EXPIRED</div><div class='kpi-value'>{expired_policies}</div><div class='kpi-sub'>Needs Review</div></div>", unsafe_allow_html=True)
